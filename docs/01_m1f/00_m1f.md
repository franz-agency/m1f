...
With v3.2+ performance improvements, m1f can handle large projects significantly faster:

**v3.2+ Performance Enhancements:**
- **Parallel Processing**: Enabled by default, provides 3-5x speedup for large file sets
- **Stream-based Checksums**: Constant memory usage regardless of file size
- **Optimized Deduplication**: Thread-safe checksum verification with minimal overhead
- **Concurrent Write Limits**: Semaphore-based write limiting prevents "too many open files" errors

**Architecture Features:**
- Async I/O with aiofiles for truly asynchronous operations
- Memory-efficient streaming for large files (8KB chunks)
- Smart caching to avoid redundant operations
- Content deduplication saves space and processing time (disable with `--allow-duplicate-files`)
- Deterministic file ordering maintained despite parallel processing

For extremely large directories with tens of thousands of files, v3.2 parallel processing provides substantial performance improvements while maintaining consistent output.
