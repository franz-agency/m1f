...
The primary output specified with `-o` contains the combined content of all processed files.

### Auxiliary Files (Automatically Generated)

For each m1f operation, auxiliary files are automatically created alongside the main output file:
...
The generated file lists can be edited and used as input for subsequent m1f operations:
...
Use `--minimal-output` to create only the main output file without auxiliary files:
