...
class TestConcurrentAssetDownloads:
    ...
    async def test_assets_download_concurrently(self, tmp_path):
        ...
        mock_scraper.extract_asset_urls = Mock(return_value=[
        ...
        with patch('tools.scrape_tool.crawlers.create_scraper', return_value=mock_scraper):
        ...
        assert total_time < 0.3, f"Downloads took {total_time}s, expected < 0.3s for concurrent downloads"
    ...
    async def test_no_delay_between_assets(self, tmp_path):
        ...
        for i in range(1, len(download_timestamps)):
            ...
            assert time_diff < 0.5, f"Asset downloads had {time_diff}s delay, expected no delay"
    ...
    async def test_batch_processing_respects_limits(self, tmp_path):
        ...
        mock_scraper.extract_asset_urls = Mock(return_value=[
            f"http://example.com/script{i}.js" for i in range(10)
        ])
