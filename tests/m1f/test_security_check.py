...
def test_security_detection():
    ...
    try:
        ...
        assert password_findings[0]["path"] == "password_file.txt", "Finding should reference correct file"
        ...
        assert api_key_findings[0]["path"] == "api_key_file.txt", "Finding should reference correct file"
        
        # Test 4: Scan all files together
        all_findings = _scan_files_for_sensitive_info([clean_tuple, password_tuple, api_key_tuple])
        
        # The API key file triggers both "Secret Keyword" and "Hex High Entropy String" detections
        assert len(all_findings) == 3, "Should have 3 findings (password + api_key with 2 detections)"
        ...
        for finding in all_findings:
            if finding["path"] == "password_file.txt":
                ...
                assert finding["type"] == "Secret Keyword", "Password should be detected as Secret Keyword"
            elif finding["path"] == "api_key_file.txt":
                api_key_findings_count += 1
                assert finding["type"] in ["Secret Keyword", "Hex High Entropy String"], "API key should be detected as either Secret Keyword or Hex High Entropy String"
        
        assert password_findings_count == 1, "Should have exactly 1 finding for password file"
        assert api_key_findings_count == 2, "Should have exactly 2 findings for API key file (both Secret Keyword and Hex High Entropy String)"
