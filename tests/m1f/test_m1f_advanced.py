...
class TestM1FAdvanced(BaseM1FTest):
    ...
    def test_large_file_handling(self, run_m1f, create_test_file, temp_dir):
        ...
        file_content_sections = [
            ...
            if section.strip() and not section.strip().startswith("size_limit_output.log")
        ]
        
        # Large file should not appear in any content section
        large_file_in_content = any("large_file.txt" in section for section in file_content_sections)
        assert not large_file_in_content, f"large_file.txt found in content sections when it should be excluded due to size limit"
