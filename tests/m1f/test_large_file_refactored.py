...
class TestLargeFileHandlingRefactored:
    ...
    def _create_large_test_file(self, file_path: Path, size_mb: float = 1.0) -> Path:
        ...
        if current_size_bytes < target_size_bytes:
            ...
            line_template = "Line {}: This is a sample line of text for performance testing."
            line_num = 0
            ...
            while current_size_bytes < target_size_bytes and iterations < max_iterations:
                line = line_template.format(line_num) + "\n"
                line_bytes = len(line.encode('utf-8'))
                
                # Check if adding this line would exceed our target
                if current_size_bytes + line_bytes > target_size_bytes * 1.1:  # Allow 10% overage
                    break
                ...
                if iterations % 10000 == 0:
                    ...
                    print(f"  Progress: {progress:.1f}% ({current_size_bytes}/{target_size_bytes} bytes)")
                    
            if iterations >= max_iterations:
                print(f"Warning: Reached maximum iterations ({max_iterations}) while creating test file")
    ...
    def _run_m1f_with_input_file(self, input_file_path: Path, output_file: Path, **kwargs) -> float:
        """
    ...
    def _run_m1f(self, arg_list):
        ...
        def mock_input(prompt=None):
            ...
            if prompt and any(word in prompt.lower() for word in ['overwrite', 'continue', 'proceed', 'y/n', '(y/n)']):
                return "y"
    ...
    def _verify_file_content(self, file_path: Path, expected_patterns: Dict[str, str]) -> None:
        ...
        for pattern_name, pattern in expected_patterns.items():
            assert pattern in content, f"Expected pattern '{pattern_name}' not found: {pattern}"
    ...
    def test_large_file_size_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for size_mb in test_sizes:
                ...
                assert output_size_mb >= size_mb * 0.9, f"Output file seems too small for {size_mb}MB input"
    ...
    def test_large_file_content_integrity(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert f"Line {num_lines-1:04d}: This is test line number {num_lines-1}" in output_content
    ...
    def test_m1f_smoke_test(self):
        ...
        args = [
            "--input-file", str(input_list_file),
            "--output-file", str(output_file),
            "--force"
        ]
