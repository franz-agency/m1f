...
class TestCrossPlatformPaths(BaseM1FTest):
    ...
    def test_bundle_creation_and_extraction(self, run_m1f, temp_dir):
        ...
        (source_dir / "src" / "components" / "ui.py").write_text("# UI component\nclass Button: pass\n")
        ...
        assert "src/components/ui.py" in bundle_content, "Expected 'src/components/ui.py' in bundle"
        ...
        result = subprocess.run(
            [sys.executable, "-m", "tools.s1f", str(output_file), "-d", str(extract_dir), "-f"],
            capture_output=True,
            text=True
        )
        ...
        assert (extract_dir / "src" / "components" / "ui.py").exists(), "src/components/ui.py not extracted"
        assert (extract_dir / "docs" / "README.md").exists(), "docs/README.md not extracted"
        
        # Verify content matches
        assert (extract_dir / "src" / "main.py").read_text() == "# Main file\nprint('Hello')\n"
        assert (extract_dir / "src" / "components" / "ui.py").read_text() == "# UI component\nclass Button: pass\n"
        assert (extract_dir / "docs" / "README.md").read_text() == "# Documentation\nTest project\n"
    ...
    def test_separator_styles(self, run_m1f, temp_dir):
        ...
        for style in separator_styles:
            ...
            assert exit_code == 0, f"m1f failed for {style} with exit code {exit_code}: {log_output}"
            ...
            if style == "MachineReadable":
                ...
                assert '"original_filepath": "test.txt"' in bundle_content or \
                ...
                for line in lines:
                    ...
                    if "FILE:" in line or "test.txt" in line.lower():
                        assert "\\" not in line, f"Found backslash in {style} style: {line}"
