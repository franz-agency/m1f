...
def test_no_default_excludes():
    ...
    args2 = parser.parse_args(["--source-directory", ".", "--output-file", "test.txt", "--no-default-excludes"])
    ...
    args3 = parser.parse_args(["--source-directory", ".", "--output-file", "test.txt", "--no-default-excludes", "--excludes", "node_modules"])
