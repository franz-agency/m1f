...
from tools.m1f.config import Config, OutputConfig, FilterConfig, EncodingConfig, SecurityConfig, ArchiveConfig, LoggingConfig, PresetConfig, SeparatorStyle, LineEnding
...
class TestParallelProcessing:
    ...
    @pytest.mark.asyncio
    async def test_parallel_processing_enabled(self, config_parallel, create_test_files):
        ...
        assert result.files_processed >= 20, f"Expected at least 20 files, got {result.files_processed}"
    ...
    @pytest.mark.asyncio
    async def test_parallel_maintains_file_order(self, config_parallel, create_test_files):
        ...
        for i, (pos, filename) in enumerate(positions):
            ...
            assert filename == expected_filename, f"File order mismatch at position {i}: expected {expected_filename}, got {filename}"
    ...
    async def test_parallel_performance_improvement(self, temp_dir, create_test_files):
        ...
        start_seq = time.time()
        files_to_process = [(temp_dir / f"large_file_{i}.txt", f"large_file_{i}.txt") for i in range(10)]
        await writer_seq._write_combined_file_sequential(config_seq.output.output_file, files_to_process)
        ...
        config_par = Config(
            ...
            input_include_files=[],
            output=OutputConfig(
                output_file=temp_dir / "output_par.txt",
                parallel=True
            ),
            filter=FilterConfig(
                include_extensions={".txt"}
        ...
        start_par = time.time()
        await writer_par._write_combined_file_parallel(config_par.output.output_file, files_to_process)
    ...
    async def test_parallel_thread_safety(self, config_parallel, temp_dir):
        ...
        content_count = output_content.count("This is duplicate content for testing deduplication")
        
        # Should only appear once due to deduplication
        assert content_count == 100, f"Duplicate content appeared {content_count} times, expected 100 (once)"
        ...
        assert "duplicate_0.txt" in output_content or "duplicate_1.txt" in output_content
    ...
    async def test_parallel_single_file_fallback(self, config_parallel, temp_dir):
        ...
        await output_writer.write_combined_file(config_parallel.output.output_file, files)
