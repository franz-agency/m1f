...
def test_exotic_encoding_conversion():
    ...
    encoding_map = {
        ...
        "windows1256.txt": ["cp1256", "windows-1256", "utf-8"],  # Sometimes detected as UTF-8
    }
    ...
    try:
        ...
        try:
            ...
            for filename, possible_encodings in encoding_map.items():
                ...
                metadata_section = content[file_content_start:file_content_start + 1000]
                ...
                normalized_allowed = [e.lower().replace('_', '-') for e in possible_encodings]
                
                assert any(normalized_detected == allowed for allowed in normalized_allowed), \
