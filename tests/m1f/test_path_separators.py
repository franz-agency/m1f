...
class TestPathSeparators:
    ...
    def test_m1f_path_normalization(self, tmp_path):
        ...
        result = get_relative_path(file_path, base_path)
        assert result == "src/components/ui/button.js", f"Expected 'src/components/ui/button.js', got '{result}'"
        ...
        assert "/" in result and "\\" not in result, f"Expected forward slashes in '{result}'"
    ...
    def test_path_object_behavior(self):
        ...
        assert "\\" not in path_obj.as_posix(), "as_posix() should not contain backslashes"
    ...
    def test_path_conversion_edge_cases(self):
        ...
        assert result == "C:/Users/test/file.txt", f"Expected 'C:/Users/test/file.txt', got '{result}'"
        
        # Test UNC paths
        result = convert_to_posix_path("\\\\server\\share\\file.txt")
        assert result == "//server/share/file.txt", f"Expected '//server/share/file.txt', got '{result}'"
        
        # Test paths with multiple consecutive separators
        result = convert_to_posix_path("path\\\\to\\\\\\file.txt")
        assert result == "path//to///file.txt", f"Expected 'path//to///file.txt', got '{result}'"
