...
class TestHTML2MDConversion:
    ...
    async def test_basic_conversion(self, test_server, temp_output_dir):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{test_server.base_url}/page/m1f-documentation") as resp:
    ...
    async def test_content_selection(self, test_server, temp_output_dir):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{test_server.base_url}/page/html2md-documentation") as resp:
                html_content = await resp.text()
    ...
    async def test_complex_layouts(self, test_server, temp_output_dir):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{test_server.base_url}/page/complex-layout") as resp:
                html_content = await resp.text()
    ...
    async def test_code_examples(self, test_server, temp_output_dir):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{test_server.base_url}/page/code-examples") as resp:
                html_content = await resp.text()
...
class TestCLI:
    ...
    def test_cli_basic_conversion(self, test_server, temp_output_dir):
        ...
        result = subprocess.run(
            [
                sys.executable, "tools/html2md.py",
                "--source-dir", f"{test_server.base_url}/page",
                "--destination-dir", temp_output_dir,
                "--include-patterns", "m1f-documentation",
                "--verbose"
            ],
            capture_output=True,
            text=True
    ...
    def test_cli_with_selectors(self, test_server, temp_output_dir):
        ...
        result = subprocess.run(
            [
                ...
                "--include-patterns", "html2md-documentation"
            ],
            capture_output=True,
            text=True
        )
