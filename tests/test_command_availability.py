        ...
        try:
            if platform.system() == "Windows":
                result = subprocess.run(
                    ["where", cmd_var], capture_output=True, text=True, check=False
                )
            else:
                result = subprocess.run(
                    ["which", cmd_var], capture_output=True, text=True, check=False
                )
...
class TestCommandAvailability:
    ...
    def test_command_exists(self, command):
        ...
        ), f"Command '{command}' not found in PATH or virtual environment"
    ...
    def test_command_help(self, command):
        ...
        try:
            ...
            assert (
                len(output) > 0
            ), f"Command '{command}' produced no output with --help"

            # Check for common help text indicators
            help_indicators = ["usage:", "Usage:", "help", "Help", "--help", "options:"]
            assert any(
                indicator in output for indicator in help_indicators
