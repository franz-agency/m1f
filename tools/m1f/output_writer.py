...
class OutputWriter:
    ...
    async def write_combined_file(
        ...
        try:
            ...
            with open(output_path, 'w', encoding=output_encoding, 
                     newline=self.config.output.line_ending.value) as outfile:
                ...
                for i, (file_path, rel_path) in enumerate(all_files, 1):
                    ...
                    if await self._write_single_file(outfile, file_path, rel_path, i, len(all_files)):
                        files_written += 1
    ...
    async def _write_single_file(
        ...
        try:
            ...
            if self.config.logging.verbose:
                self.logger.debug(f"Processing file ({file_num}/{total_files}): {file_path.name}")
            ...
            if (content and 
                ...
                self.config.output.separator_style != SeparatorStyle.MACHINE_READABLE):
                outfile.write(self.config.output.line_ending.value)
            ...
            if (file_num < total_files and 
                self.config.output.separator_style != SeparatorStyle.NONE):
                outfile.write(self.config.output.line_ending.value)
