    ...
    def get_preset_for_file(self, file_path: Path) -> Optional[FilePreset]:
        ...
        for preset_name, preset in self.file_presets.items():
            ...
            if preset.extensions:
                if file_path.suffix.lower() in [ext.lower() if ext.startswith('.') else f'.{ext.lower()}' 
                                                for ext in preset.extensions]:
                    logger.debug(f"File {file_path} matched extension in preset {preset_name}")
            ...
            if preset.patterns:
                for pattern in preset.patterns:
                    if fnmatch.fnmatch(str(file_path), pattern):
                        logger.debug(f"File {file_path} matched pattern '{pattern}' in preset {preset_name}")
                        matched_preset = preset
                        break
    ...
    def _merge_with_globals(self, preset: FilePreset, file_path: Path) -> FilePreset:
        ...
        if global_preset.preserve_tags:
            if merged.preserve_tags:
                merged.preserve_tags = list(set(merged.preserve_tags + global_preset.preserve_tags))
            else:
...
class PresetManager:
    ...
    def load_preset_file(self, preset_path: Path) -> None:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Preset file must contain a dictionary, got {type(data)}")
            ...
            for group_name, group_data in data.items():
                ...
                logger.info(f"Loaded preset group '{group_name}' with {len(group.file_presets)} presets")
    ...
    def _parse_group(self, name: str, data: Dict[str, Any]) -> PresetGroup:
        ...
        if 'global_settings' in data:
            ...
            if 'include_extensions' in global_data:
                group.global_settings.include_extensions = global_data['include_extensions']
            ...
            if 'extensions' in global_data:
                for ext, preset_data in global_data['extensions'].items():
                    ...
                    group.global_settings.extension_settings[ext] = FilePreset.from_dict(preset_data)
    ...
    def get_preset_for_file(self, file_path: Path, group_name: Optional[str] = None) -> Optional[FilePreset]:
    ...
    def process_content(self, content: str, preset: FilePreset, file_path: Path) -> str:
        ...
        for action in preset.actions:
            if action == ProcessingAction.NONE:
                ...
                content = self._strip_tags(content, preset.strip_tags, preset.preserve_tags)
    ...
    def _strip_tags(self, content: str, tags_to_strip: List[str], preserve_tags: List[str]) -> str:
        ...
        try:
            ...
            logger.warning("BeautifulSoup not installed - using regex fallback for tag stripping")
    ...
    def _apply_custom_processor(self, content: str, processor_name: str, 
                               args: Dict[str, Any], file_path: Path) -> str:
    ...
    def _processor_truncate(self, content: str, args: Dict[str, Any], file_path: Path) -> str:
    ...
    def _processor_redact_secrets(self, content: str, args: Dict[str, Any], file_path: Path) -> str:
    ...
    def _processor_extract_functions(self, content: str, args: Dict[str, Any], file_path: Path) -> str:
...
def load_presets(preset_paths: Union[Path, List[Path]], 
                auto_load_user_presets: bool = True) -> PresetManager:
    ...
    for name, group in sorted_groups:
        ...
        if group.globals and group.globals.extension_defaults:
            ...
            for ext, preset in group.globals.extension_defaults.items():
                actions = [a.value for a in preset.actions] if preset.actions else ["none"]
