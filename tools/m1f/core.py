...
from .exceptions import FileNotFoundError, PermissionError, ValidationError, SecurityError
    ...
    async def run(self) -> ProcessingResult:
        ...
        try:
            ...
            if self.config.security.security_check:
                ...
                files_to_process = self._handle_security_results(files_to_process, flagged_files)
            
            # Generate content hash if requested
            if self.config.output.filename_mtime_hash:
                output_path = await self._add_content_hash_to_filename(output_path, files_to_process)
            ...
            if not self.config.output.skip_output_file:
                ...
                self.logger.info(f"Successfully combined {files_processed} files into '{output_path}'")
                
                # Count tokens if available
                token_count = await self._count_tokens(output_path)
                if token_count:
                    self.logger.info(f"Output file contains approximately {token_count} tokens")
            else:
            ...
            if self.config.security.security_check and self.config.security.security_check.value == "warn" and flagged_files:
            ...
            return ProcessingResult(
                ...
                output_file=output_path if not self.config.output.skip_output_file else None,
                archive_file=archive_path,
                token_count=token_count if not self.config.output.skip_output_file else None,
    ...
    async def _prepare_output_path(self) -> Path:
        ...
        if output_path.exists() and not self.config.output.skip_output_file:
            if self.config.output.force_overwrite:
                ...
                if not self.config.logging.quiet:
