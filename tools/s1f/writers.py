...
class FileWriter:
    ...
    async def write_files(self, extracted_files: List[ExtractedFile]) -> ExtractionResult:
    ...
    async def _write_file_async(self, file_data: ExtractedFile, result: ExtractionResult):
        ...
        try:
            ...
            # Write the file
            content_bytes = await self._encode_content(file_data.content, encoding, file_data.path)
    ...
    async def _write_file_sync(self, file_data: ExtractedFile, result: ExtractionResult):
    ...
    async def _encode_content(self, content: str, encoding: str, file_path: str) -> bytes:
    ...
    async def _set_file_timestamp(self, path: Path, file_data: ExtractedFile):
        ...
        if (self.config.timestamp_mode == "original" and 
            file_data.metadata.modified is not None):
            try:
    ...
    async def _check_line_ending_difference(self, path: Path, file_data: ExtractedFile,
                                             calculated: str, expected: str):
    ...
    def _check_line_ending_difference_sync(self, path: Path, file_data: ExtractedFile,
                                           calculated: str, expected: str):
