...
- Symlink Handling: When using `--include-symlinks`, the script follows symbolic links but ensures each physical file
  appears only once in the output. This prevents duplication even when a file is accessible via multiple symlinks.
- Duplicate Content Detection: Files with identical content are automatically detected and only included once in the output,
  regardless of path, filename, or modification time. This eliminates redundancy in the final output.
...
def _detect_symlink_cycles(
    ...
    try:
        ...
        while current.is_symlink():
            if depth >= max_depth:
                logger.debug(
                    "Symlink resolution depth %s exceeded at %s – treating as cycle",
                    max_depth,
                    current,
                )
            ...
            try:
                if current.parent.resolve(strict=False).is_relative_to(target):
                    logger.debug(
                        "Symlink cycle detected: %s → %s is ancestor of current path",
                        current,
                        target,
                    )
                    return True, visited_paths
...
def _build_exclusion_set(
    ...
    # Only include default excludes if use_default_excludes is True
...
def _is_file_excluded(
    ...
    if not explicitly_included:
        # Only exclude dot files if include_dot_paths is not set
        if not args.include_dot_paths and file_path.name.startswith("."):
            # When no_default_excludes is set, don't exclude .git specifically
    ...
    # With no_default_excludes, we still need to check against excluded_dir_names_lower
    # for directories explicitly excluded with --excludes
    p = file_path.parent
    while p != p.parent:  # Stop when p is the root (e.g. '/' or 'C:\')
        if p.name and p.name.lower() in excluded_dir_names_lower:
            ...
            if (
                ...
                if args.verbose:
                    logger.debug(
                        f"Not excluding file '{file_path}' in {p.name} directory because no_default_excludes is set."
                    )
                pass  # Just continue checking parents
            else:
                if args.verbose:
                    logger.debug(
...
def _gather_files_to_process(
    ...
    if getattr(args, "include_symlinks", False):
        # When --include-symlinks is active, we still want to deduplicate by canonical path
        # to avoid any file appearing twice, regardless of how it's accessed (via symlink or direct)
    ...
    if args.verbose:
        if ignore_symlinks:
            logger.debug("Symlinks will be ignored")
        else:
            logger.debug("Symlinks will be included (if they don't form cycles)")
    ...
    if input_paths is not None:
            ...
            if item_path.is_file():
                ...
                if item_path.name.lower() in excluded_dir_names_lower:
                    ...
                    if getattr(
                        ...
                        # Continue processing this directory
                    else:
                        logger.debug(
                ...
                if not ignore_symlinks and item_path.is_symlink():
                    ...
                    symlink_visited_paths = updated_visited
                    logger.debug(f"Including symlink directory: {item_path}")
        ...
        for root, dirs, files in os.walk(
            source_dir,
            followlinks=not ignore_symlinks,  # follow symlinks when requested
        ):
            root_path = Path(root)
            ...
            if hasattr(args, "no_default_excludes") and args.no_default_excludes:
                ...
                # Normal behavior - exclude all directories in excluded_dir_names_lower
            ...
            for file in files:
                ...
                if file_path.is_symlink():
                    ...
                    if is_cycle:
                        logger.debug(
                            f"Skipping symlink file that would cause cycle: {file_path}"
                        )
                        continue
...
def _generate_file_content_checksum(file_path: Path) -> str:
    ...
    try:
        ...
        return f"ERROR_CHECKSUM_{str(file_path)}_{uuid.uuid4().hex}"
...
def _write_combined_data(
    ...
    try:
        ...
        # Open output file for writing (no special-casing for tests)
        with open(
            output_file_path,
            "w",
            ...
            for file_counter, (file_info, rel_path_str) in enumerate(
                ...
                if should_deduplicate_by_content and not rel_path_str.startswith(
                    ...
                    if content_checksum in content_checksums:
                        ...
                        content_checksums[content_checksum] = file_info
...
def main():
  ...
  %(prog)s -s ./src -o combined.txt --skip-output-file""",
    ...
    parser.add_argument(
        "--include-symlinks",
        action="store_true",
        help="Include symbolic links to files and directories. Default behavior is to skip symlinks. Files will be deduplicated based on their physical location, so each file appears only once in the output even if reachable via multiple symlinks. CAUTION: This might cause infinite recursion if symlinks form cycles!",
    )
    parser.add_argument(
        "--separator-style",
