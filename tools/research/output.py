...
class Colors:
    """Color codes for terminal output using colorama when available"""
    ...
    if COLORAMA_AVAILABLE:
        ...
        # Fallback to empty strings when colorama not available
        BLACK = ""
        RED = ""
        GREEN = ""
        YELLOW = ""
    ...
    def disable(cls):
        ...
        for attr in dir(cls):
            if not attr.startswith("_") and attr not in ("disable", "enable"):
                setattr(cls, attr, "")
...
class OutputFormatter:
    ...
    def print(self, message: str = "", level: str = "info", **kwargs):
    ...
    def job_status(self, job: Dict[str, Any]):
        ...
        self.print(f"{Colors.BOLD}Status:{Colors.RESET} ", end="")

        # Color-code status
        status = job["status"]
        if status == "completed":
            self.print(f"{Colors.GREEN}{status}{Colors.RESET}")
        elif status == "active":
            self.print(f"{Colors.YELLOW}{status}{Colors.RESET}")
        else:
            self.print(f"{Colors.RED}{status}{Colors.RESET}")
