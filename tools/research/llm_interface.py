...
class ClaudeCodeProvider(LLMProvider):
    ...
    async def query(
        ...
        try:
            ...
            # Use verbose and stream-json for real-time feedback
            cmd = [
                self.binary_path,
                "-p",
                "--allowedTools",
                    ...
                    if line:
                        ...
                        if show_progress:
                            ...
                            try:
                                ...
                                if (
                                    msg_type == "tool_use"
                                    and json_obj.get("name") == "WebSearch"
                                ):
                                    query_info = json_obj.get("input", {}).get(
                                        "query", ""
                                    )
                                    sys.stdout.write(
                                        "\r" + " " * 80 + "\r"
                                    ...
                                    info(
                                        f"  🔍 Searching web for: {query_info[:60]}..."
                                    )
                                elif (
                                    msg_type == "tool_response"
                                    and "search_results" in line
                                ):
                                    sys.stdout.write(
                                        "\r" + " " * 80 + "\r"
                                    )  # Clear line
                                    info(f"  ✅ Found search results")
                                elif msg_type == "assistant":
                                    # Check for content mentioning URLs
