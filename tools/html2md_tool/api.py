...
class Html2mdConverter:
    ...
    def convert_html(
        ...
        # First, extract all H1 tags and title before applying selectors
        h1_tags = parsed.find_all("h1")
        h1_contents = []

        # Store all H1 contents
        for h1 in h1_tags:
        ...
        if self.config.conversion.outermost_selector:
            ...
            if selected:
                ...
                # If we had H1s before but they're gone now, restore them
                if h1_contents and not existing_h1s_after:
                    # Add the first H1 content as H1
                ...
                elif not h1_contents and not existing_h1s_after and title_content:
                    # No H1 at all, create one from title
                    new_h1 = parsed.new_tag("h1")
