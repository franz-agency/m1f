...
class Html2mdConverter:
    ...
    def convert_html(
        ...
        # First, extract H1 and title before applying selectors
        h1_content = None
        title_content = None

        # Try to find H1 anywhere in the document
        h1_tag = parsed.find("h1")
        if h1_tag:
            h1_content = str(h1_tag)
        ...
        if self.config.conversion.outermost_selector:
            ...
            if selected:
                ...
                if not parsed.find("h1") and h1_content:
                    # H1 was removed by selector, prepend it
                    h1_soup = BeautifulSoup(h1_content, "html.parser")
                    parsed.insert(0, h1_soup.h1)
                elif not parsed.find("h1") and title_content:
