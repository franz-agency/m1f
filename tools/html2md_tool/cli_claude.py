...
def handle_claude_analysis_improved(html_files: List[Path], num_files_to_analyze: int = 5, parallel_workers: int = 5, project_description: str = ""):
...
{simple_prompt_template}"""
    ...
    for f in selected_files:
        if any(word in f.lower() for word in ["select", "based on", "analysis", "representative"]) or len(f) > 200:
            continue
    ...
    if not individual_prompt_path.exists():
        console.print(f"❌ Prompt file not found: {individual_prompt_path}", style="red")
        return
    ...
    for result in results:
        if not result['success']:
            console.print(f"❌ Failed: {result['name']} - {result.get('error') or result.get('stderr')}", style="red")
...
- m1f/analysis/html_analysis_5.txt"""
    ...
    try:
        ...
        if yaml_start != -1 and yaml_end != -1:
            ...
            console.print(f"  m1f-html2md convert {common_parent} -c {config_path} -o ./output/", style="blue")
        else:
            console.print("\n⚠️  Could not extract YAML config from Claude's response", style="yellow")
            console.print("Please review the output above and create the config file manually.", style="yellow")
