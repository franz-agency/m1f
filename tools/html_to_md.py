...
def configure_logging(
    verbose: bool, output_dir: Optional[Path] = None, quiet: bool = False
) -> None:
...
def read_file_with_encoding(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Encoding issues with {file_path}. Falling back to UTF-8 with replacement."
            )
            used_encoding = "utf-8"
...
def adjust_internal_links(
    soup: BeautifulSoup, link_extensions: Optional[Dict[str, str]] = None
) -> None:
...
def create_frontmatter(
    ...
    if not YAML_AVAILABLE:
        logger.warning(
            "yaml module not available. Frontmatter will be created manually."
        )
...
def convert_html(
    ...
    if outer_selector:
        ...
        if found:
            ...
            logger.warning(
                f"Selector '{outer_selector}' not found. Using entire document."
            )
    ...
    # Convert to Markdown
    md_options = {"heading_style": "ATX"}  # Use # style headings
            ...
            if not in_code_block and line.strip() == "```":
                ...
                if code_block_end is not None:
                    ...
                    for pre_elem in soup_temp.find_all("pre"):
                        ...
                        if (
                            code_elem
                            and code_elem.get_text().strip() == code_content.strip()
                        ):
                            if code_elem.has_attr("class"):
                                for cls in code_elem["class"]:
                                    if cls.startswith(("language-", "lang-")):
                                        ...
                                        result_lines.extend(
                                            lines[code_block_start:code_block_end]
                                        )
                                        result_lines.append("```")
                                        i = code_block_end + 1
                                        break
    ...
    if add_line_breaks:
        ...
        md_content = md_content.replace(
            "\n1.", "\n\n1."
        )  # Add space before ordered lists
        md_content = md_content.replace(
            "\n---", "\n\n---"
...
def main() -> None:
    ...
    parser.add_argument(
        "--remove-elements",
        nargs="*",
        default=DEFAULT_REMOVE_ELEMENTS,
    ...
    parser.add_argument(
        "--exclude-dirs", nargs="*", help="Directory names to exclude from processing"
    )
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Suppress all console output"
    )
    ...
    if not src_root.exists() or not src_root.is_dir():
        logger.error(
            f"Source directory does not exist or is not a directory: {src_root}"
        )
        sys.exit(1)
